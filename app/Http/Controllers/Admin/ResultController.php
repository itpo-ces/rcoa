...
class ResultController extends Controller
{
    ...
    private function getActionButtons($examinee)
    {
        return '<div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-outline-primary" 
                data-toggle="modal" 
                data-target="#edit_item_modal" 
                data-item="' . $examinee->id . '">
                <i class="fas fa-eye"></i> View Details
            </button>
            <a href="' . route('results.export', $examinee->id) . '" 
    ...
    {
        $examinee = Examinee::with(['responses.question', 'token'])->findOrFail($id);
        
        $totalQuestions = $examinee->responses->groupBy('question_id')->count();
        $score = $examinee->responses->sum('is_correct');
        $percentage = $totalQuestions > 0 ? round(($score / $totalQuestions) * 100) : 0;

        $filename = 'Exam_Result_' . str_replace(' ', '_', $examinee->full_name) . '_' . now()->format('Ymd_His') . '.xlsx';

        return Excel::download(new ExamResultExport($examinee, $totalQuestions, $score, $percentage), $filename);
    }
}